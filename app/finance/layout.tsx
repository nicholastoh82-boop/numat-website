...
export default function FinanceLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!user) { router.replace('/crm/login'); return; }
      ...
      <header className="border border-gray-200 border-t-0 border-x-0 bg-white">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-6">
            <Link href="/finance" className="text-xl font-semibold tracking-tight">NUMAT Finance</Link>
            <nav className="flex gap-1">
              {navItems.map(item => {
                const active = pathname === item.href || (item.href !== '/finance' && pathname?.startsWith(item.href));
                return (
                  <Link key={item.href} href={item.href}
                    className={`px-3 py-1.5 rounded text-sm ${active ? 'bg-gray-900 text-white' : 'text-gray-700 hover:bg-gray-100'}`}>
                    {item.label}
                  </Link>
                );
              })}
          ...
          <div className="flex items-center gap-3 text-sm text-gray-600">
            <span>{userEmail}</span>
            <button onClick={async () => { await supabase.auth.signOut(); router.replace('/crm/login'); }}
              className="text-gray-500 hover:text-gray-900">Sign out</button>
          </div>
        </div>
      </header>
