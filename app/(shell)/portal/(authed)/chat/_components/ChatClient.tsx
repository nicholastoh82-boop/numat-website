...
export default function ChatClient() {
  ...
  function seersFor(msg: Message): { id: string; name: string; at: string }[] {
    const t = new Date(msg.created_at).getTime()
    return reads
      .filter((r) => r.user_id !== msg.sender_id && new Date(r.last_read_at).getTime() >= t)
      .map((r) => {
        const m = members.find((x) => x.id === r.user_id)
        const name = r.user_id === userId ? userName || 'You' : m?.name || m?.email || 'Member'
        return { id: r.user_id, name, at: r.last_read_at }
      })
    ...
    <div className="flex flex-col h-[calc(100dvh-7rem)] md:h-[calc(100dvh-9rem)]">
                          ...
                          >
                            ...
                            {mine && m.id === lastOwnId ? (
                              <button
                                ...
                                className="text-[10px] mt-0.5 text-gray-300 hover:underline"
                              >
      ...
      {seenByMsg ? (
        <div
          ...
          onClick={() => setSeenByMsg(null)}
        >
          <div
            className="bg-white w-full sm:max-w-xs rounded-t-2xl sm:rounded-lg shadow-lg max-h-[70dvh] flex flex-col"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="px-4 py-3 border-b border-gray-200">
            ...
            <div className="flex-1 overflow-y-auto p-2">
              {seersFor(seenByMsg).length === 0 ? (
                <p className="text-sm text-gray-500 p-3">No one has seen this yet.</p>
              ) : (
                seersFor(seenByMsg).map((s) => (
                  <div key={s.id} className="flex items-center justify-between gap-2 px-2 py-2">
                    <span className="text-sm text-gray-900 truncate">{s.name}</span>
                    <span className="text-[11px] text-gray-400 shrink-0">{timeLabel(s.at)}</span>
                  </div>
                ))
              )}
            </div>
