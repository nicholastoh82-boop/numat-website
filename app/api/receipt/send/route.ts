...
export async function POST(req: NextRequest) {
  ...
  // 2. Parse body
  let body: { receipt_id?: string; recipient_email?: string; recipient_name?: string; sender_rep_email?: string; sent_by?: string };
  ...
  try {
    ...
    // 5. Generate the receipt PDF (existing endpoint)
    const pdfRes = await fetch(`https://numatbamboo.com/api/receipt/${encodeURIComponent(receipt.id)}/pdf`);
