...
const EMAIL_RATE_LIMIT_WINDOW = 60000
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!checkEmailRateLimit(quoteId)) {
      console.warn(`[Rate Limit] Too many email sends for quote: ${quoteId}`)
    ...
    if (channel === 'whatsapp') {
      return NextResponse.json(
        { error: 'Use /api/admin/send-whatsapp-quote for WhatsApp messages' },
        { status: 400 }
      )
    }

    // Fetch quote + items directly (no customers join)
    ...
    // Email is stored directly on quotes row
    const customerEmail = typedQuote.email
    const customerName = typedQuote.customer_name ?? 'Valued Customer'

    if (!customerEmail) {
      console.error('No email found on quote record:', quoteId)
      return NextResponse.json({ error: 'Customer email not found on quote record' }, { status: 400 })
    }
    ...
    await sendEmail({
      ...
      subject:
        type === 'reminder'
          ...
          : `Your Quote #${typedQuote.quote_number} from NUMAT Bamboo`,
      html: htmlContent,
    ...
    await supabaseAdmin.from('quotes').update(updateData).eq('id', quoteId)

    // Log communication if table exists
    try {
      ...
      console.warn('quote_communications log failed (non-blocking):', logErr)
    }
