...
export default function BoardLoginForm() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setError(null)
    try {
      const res = await fetch('/api/board-portal/auth', {
        method: 'POST',
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-white px-4">
      <form onSubmit={submit} className="w-full max-w-sm rounded-2xl border border-gray-200 p-8 shadow-sm">
        <h1 className="text-xl font-semibold text-gray-900">NUMAT Board View</h1>
        ...
        <input
          ...
          autoFocus
          className="mt-5 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-gray-900 focus:outline-none"
        />
