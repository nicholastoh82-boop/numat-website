...
- The sheet must be a native Google Sheet (not an .xlsx in Drive) shared as Editor with the service account `gemini-cron-runner@numat-automation.iam.gserviceaccount.com`. Auth uses the existing Workload Identity Federation helper `lib/cron/gcp_auth.ts`, which only works on Vercel.
